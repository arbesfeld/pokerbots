...
public class Brain {
	...
	private Action actPreFlopNotButton() { //big blind acts second
		if(eL(1)) {
			...
			if(maj.pfAggro() < 1.0 - equity) {
				return checkFold();
			}
			else if(maj.pfAggro() < (1.0 - equity) * 1.25) {
				return call();
			}
			else if(maj.pfAggro() < (1.0 - equity) * 1.5) {
				return putPotPercentage(equity, maj.pfAggro(), (1.0 - equity) * 1.5, 
						pVals[1], pVals[2] ); 
			}
			else {
				return putPotPercentage(equity, maj.pfAggro(), (1.0 - equity) * 1.5, 
			...
			else if(maj.pfAggro() < (1.0 - equity) * 1.33) {
				return putMin();
			}
			else if(maj.pfAggro() < (1.0 - equity) * 1.66) {
				return putPotPercentage( equity, maj.pfAggro(), (1.0 - equity) * 1.66, 
						pVals[3], pVals[4]); 
			}
			else {
				return putPotPercentage(equity, maj.pfAggro(), (1.0 - equity) * 1.66, 
	...
	private Action actPostFlopNotButton() { //acts first
		if(eL(3)) {
			...
			if(dory.hasOpponentRaisedThisStreet()) {
				if(dory.lastOpponentRaiseSize() > pVals[1])
					return fold();
				else
					return call();
			}
