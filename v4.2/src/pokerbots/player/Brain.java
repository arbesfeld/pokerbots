...
public class Brain {
	...
	Brain(Historian maj, Card[] hand, double timebank, boolean callRaise, boolean checkRaise) {
		...
		pVals = new double[]{0.15, 0.25, 0.4, 0.55, 0.7, 0.9, 1.1, 1.3, 1.5, 1.7, 1.9, 2.1, 2.3, 2.5};
	}
	public void reset() {
		raiseAction = null;
		betAction = null;
	...
	public Action act() {
		...
		System.out.println("Potodds: " + dory.potOdds());
	...
	private Action actPreFlopButton() { //small blind acts first
		...
		if(eL(1)) {
			...
			return checkFoldCallPotOdds();//return putLin(dory.liqEquity(), eVals[2], eVals[3], sVals[0], sVals[1]);
		}
		else if(eL(4)) {
	...
	private Action actPostFlopButton() {    //acts second
		if(eL(3)) {
			...
			return checkFoldCallPotOdds();//return putPotPercentage(dory.liqEquity(), eVals[3], eVals[4], pVals[2], pVals[3]);
		}
		else if(eL(5)) {
	...
	private Action checkFoldCallPotOdds() {
		if(canCheck)
			return check();
	...
	private Action foldCallPotOdds() {
	...
	private Action raisePotPercentage(double percent) {
		int raiseAmt = (int) (potSize * percent);
	...
	private Action betPotPercentage(double percent) {
		int betAmt = (int) (potSize * percent);
