...
public class Dory {
	...
	double pfRaiseFactor = 0.0005;
	double pfCallFactor = 0.05;
	double pfCheckFactor = 0.05;
	
	double aggroFactor = 0.2;
	double sdwFactor = 0.1;
	double pfrDivFactor = 2.0;
	...
	private void theirRaiseAction(PerformedAction performedAction) {
		...
		if(PFR > 0.5)
			adjustedPFR = PFR;
		double numRaises = (theirRaiseHistory[currentState.ordinal()].size() + 1.0) / 2.0;
		//if(currentState == GameState.PREFLOP) { 
			changeEquity -= littleFactor * numRaises * pfRaiseFactor * HelperUtils.logistic(maj.stackSize, maj.stackSize, (performedAction.getAmount() - theirBetsThisStreet)) / 
	...
	private void theirBetAction(PerformedAction performedAction) {
		...
		changeEquity -= littleFactor * pfRaiseFactor * HelperUtils.logistic(maj.stackSize, maj.stackSize, performedAction.getAmount()) / 
				((adjustedPFR - 0.5) / pfrDivFactor + 0.5);
