...
public class Dory {
	ArrayList<Double>[] theirRaiseHistory; //array of ArrayLists representing the raise history of the opp
								       //theirRaiseHistory[0] contains all raises of opp preflop, etc
	ArrayList<Double>[] myRaiseHistory;
	boolean[] checkHistory;   		   //has opponent checked this street?
	int[] betHistory;                  //opponent can only bet once
	double pfRaiseFactor = 0.00024;
	...
	Dory(Brain brain, Historian maj) {
		...
		for(int i = 0; i < 4; i++) {
			myRaiseHistory[i] = new ArrayList<Double>();
			theirRaiseHistory[i] = new ArrayList<Double>();
		}
	...
	private void myRaiseAction(PerformedAction performedAction) {
		...
		double potBet = (double)(performedAction.getAmount() - myBetsThisStreet) / (brain.potSize - performedAction.getAmount());
		myRaiseHistory[currentState.ordinal()].add(potBet);
	}
	...
	private void theirRaiseAction(PerformedAction performedAction) {
		...
		theirRaiseHistory[currentState.ordinal()].add(potBet);
		...
		double logFactor = HelperUtils.logisticSmall(3.0, 3.0, potBet) * 
				           (HelperUtils.logistic(400.0, 400.0, performedAction.getAmount() - theirBetsThisStreet) + 300.0) / 6;
		...
		if(myRaiseHistory[currentState.ordinal()].size() == 1 && theirRaiseHistory[currentState.ordinal()].size() == 1) {
			logFactor *= 1.0 + (0.205 - TWOB) * myRaiseHistory[currentState.ordinal()].get(0);
		}
		else if(theirRaiseHistory[currentState.ordinal()].size() == 2) {
			logFactor *= 1.0 + (0.5 - THREEB) * 1.5;
		}
		else if(theirRaiseHistory[currentState.ordinal()].size() >= 3) {
			logFactor *= 1.0 + (0.5 - THREEB) * 3.0;
		}
	...
	private void theirBetAction(PerformedAction performedAction) {
		...
		double logFactor = HelperUtils.logisticSmall(3.0, 3.0, potBet) * 
		                   (HelperUtils.logistic(400.0, 400.0, performedAction.getAmount()) + 300.0) / 6;
