...
public class Dory {
	...
	private void myPostAction(PerformedAction performedAction) {
		...
		changeEquity += (maj.getSDWRate() - 0.5) / 3;
	}
	...
	private void theirCallAction(PerformedAction performedAction) {
		...
		changeEquityCall = pfCallFactor * (adjustedPFR - 0.15) / (1.0 + theirRaiseHistory[currentState.ordinal()].size() + myRaiseHistory[currentState.ordinal()].size());
