...
public class Dory {
	...
	double postFlopRaiseFactor = 0.00033;
	...
	private void theirRaiseAction(PerformedAction performedAction) {
		...
		double THREEB = Math.min(0.35, maj.get3BetRate());
		double TWOB = Math.min(0.25, maj.get2BetRate());
		
		if(myRaiseHistory[currentState.ordinal()].size() == 1 && theirRaiseHistory[currentState.ordinal()].size() == 1) {
			logFactor *= 1.1 + (0.25 - TWOB) * 2.0;
		}
		else if(theirRaiseHistory[currentState.ordinal()].size() == 2) {
			double raiseSize = myRaiseHistory[currentState.ordinal()].get(myRaiseHistory[currentState.ordinal()].size() - 1);
			
			if(myRaiseHistory[currentState.ordinal()].size() > 1)
				raiseSize -= myRaiseHistory[currentState.ordinal()].get(myRaiseHistory[currentState.ordinal()].size() - 2);
			logFactor *= 1.1 + (0.35 - THREEB) * Math.min(100.0, raiseSize) / 20.0;
		}
			...
			if(myRaiseHistory[currentState.ordinal()].size() > 1)
				raiseSize -= myRaiseHistory[currentState.ordinal()].get(myRaiseHistory[currentState.ordinal()].size() - 2);
			logFactor *= 1.15 + (0.35 - THREEB) * Math.min(100.0, raiseSize) / 10.0;
		}
...
//		}
	...
	private void streetAction(PerformedAction performedAction) {
		...
		if(performedAction.getStreet().equalsIgnoreCase("FLOP")) {
			...
			double wetness = (double)brain.wetness;
			System.out.println("Wetness: " + wetness);
			if(wetness < 0)
				changeEquity += wetness / 150;
			
			
			//changeEquity -= (1 - brain.equity) * 0.2;
